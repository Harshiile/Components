        ...
        <div className="w-1/2 pr-4">
          ...
          <form className="space-y-6">
            ...
            <div className="relative p-1.5 bg-zinc-800 rounded-full flex shadow-inner w-full">
              <div
                className={`absolute top-1.5 left-1.5 h-[calc(100%-12px)] w-[calc(25%)] bg-white rounded-full transition-all duration-300`}
                style={{
          ...
          <button
            ...
            className="mt-4 w-full bg-white text-black font-medium py-3 px-4 rounded-lg transition-all duration-300 hover:bg-zinc-100 active:bg-zinc-200 hover:scale-[1.02] flex items-center justify-center gap-2"
          >
          ...
          {/* Subject Input Form */}
          {isSubjectOpen && (
            <div className="mt-4 p-4 bg-zinc-800 rounded-lg transition-all duration-500">
              <input
                type="text"
                placeholder="Subject Name"
                value={newSubject}
                onChange={(e) => setNewSubject(e.target.value)}
                className="w-full mb-2 px-4 py-2 bg-zinc-700 border border-white/10 rounded-lg text-white"
                required
              />
              {teachers.map((teacher, index) => (
                <input
                  key={index}
                  type="text"
                  placeholder={`Teacher ${index + 1} (Optional)`}
                  value={teacher}
                  onChange={(e) => {
              ...
              <button
                onClick={addSubject}
                className="w-full bg-white text-black font-medium py-2 px-4 rounded-lg hover:bg-zinc-100 transition-all duration-300"
              >
                Add Subject
              </button>
            </div>
        ...
        <div className="w-1/2 pl-4 text-white">
          ...
          {subjects.map((subject, index) => (
            <div key={index} className="p-4 bg-zinc-800 rounded-lg mb-2">
              ...
              <p className="text-sm text-white/70">Teachers: {subject.teachers.filter(t => t).join(", ")}</p>
            </div>
          ))}
        </div>
      </div>
